...
python3 -m examples.openhands.train_openhands \
    ...
    \
    # Proxy: keep on 127.0.0.1 (host loopback).
    # DockerSandbox automatically adds host.docker.internal→host-gateway so
    # worker_server.py inside the outer container can reach the proxy.
    ...
    \
    # Mount Docker socket into outer rllm container so OpenHands (running
    # in-process) can spawn its own runtime containers via docker runtime.
